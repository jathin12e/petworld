      ...
      <div style={{ display: 'flex', flexWrap: 'wrap',justifyContent:"center"}}>
        {data.map((u) => (
              <div key={u.id} style={{ margin: '10px', border: '0px solid #ddd', padding: '10px',cursor:'pointer' }}>
                <Link style={{color:"#000", textDecoration:"none"}} to={`/dogs/dog-detailspage/${u.id}`}><img src={`https://petworld-h7ux.onrender.com/images/${u.image}`} alt={u.name} style={{ width: '300px', height: '300px' , borderRadius:"20px" }} />
                <h3>{u.name}</h3>
