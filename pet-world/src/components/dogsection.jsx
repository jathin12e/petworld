      ...
      <div style={{ display: 'flex', flexWrap: 'wrap',justifyContent:"center"}}>
        {data.map((u) => (
              <div key={u.id} style={{ margin: '10px', border: '1px solid #ddd', padding: '10px',cursor:'pointer' }}>
                <Link style={{color:"#000", textDecoration:"none"}} to={`/dog-detailspage/${u.id}`}><img src={`http://192.168.0.107:8080/images/${u.image}`} alt={u.name} style={{ width: '300px', height: '300px'  }} />
                <h3>{u.name}</h3>
